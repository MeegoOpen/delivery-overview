...
export const handleErrorMsg = (e: any) => {
  let msg = "";
...
export const getControlContext = async () =>
  sdk.control.getContext().catch((e) => handleErrorMsg(e));
...
export const getWorkObject = (params: {
  spaceId: string;
  workObjectId: string;
}) => sdk.WorkObject.load(params);

export const getRoleList = (spaceId: string, workObjectId: string) =>
  getWorkObject({
    spaceId,
    workObjectId,
  }).then((wo) => wo.getRoleList());
